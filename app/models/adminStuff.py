    ...
    def resetEverything():
        if not current_user.is_anonymous and current_user.is_admin:
            with dbapi2.connect(current_app.config['dsn']) as connection:
                ...
                query = "CREATE TABLE USERS (USERNAME VARCHAR(50) PRIMARY KEY,PASS VARCHAR(120), ISADMIN BOOLEAN, PICTURE VARCHAR(500) DEFAULT 'https://www.wired.com/wp-content/uploads/2015/11/GettyImages-134367495.jpg', BESTFRIEND VARCHAR(50) REFERENCES USERS(USERNAME) ON DELETE SET DEFAULT DEFAULT 'admin')"
                cursor.execute(query)
